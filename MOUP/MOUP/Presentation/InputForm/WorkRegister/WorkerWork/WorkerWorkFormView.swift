...
struct WorkerWorkFormView: View {
    ...
    /// push된 하위 위저드(근무지/근무자 선택·반복 설정)가 하나라도 떠 있는지 여부.
    ///
    /// finder는 부모(`OwnerWorkRegisterView`)에 있고 위저드 플래그는 여기 있어,
    ...
    private var isFormWizardPresented: Bool {
        showWorkplaceSelect || showWorkerSelect || showRepeatSettings
    }
