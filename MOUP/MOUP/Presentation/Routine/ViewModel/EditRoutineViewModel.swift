...
final class EditRoutineViewModel {
    ...
    struct Output {
        ...
        let editCompleted: Signal<Routine>
    }
    
    // MARK: - Properties
    
    private let initialRoutine: Routine?
    private let disposeBag = DisposeBag()
    
    // MARK: - Initializer
    
    init() {
        self.initialRoutine = nil
    }
    ...
    func transform(input: Input) -> Output {
        let itemsRelay = BehaviorRelay<[TodoItem]>(
            value: {
                if let routine = initialRoutine,
        ...
        let titleRelay = BehaviorRelay<String>(value: initialRoutine?.title ?? "")
        let alarmTimeRelay = BehaviorRelay<DateComponents?>(value: initialRoutine?.alarmTime)
        ...
        input.editButtonTapped
            ...
            .subscribe(onNext: { title, alarmTime, items in
                ...
                let routineID = self.initialRoutine?.id ?? UUID()
